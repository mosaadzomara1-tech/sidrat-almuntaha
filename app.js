...
async function openPage(p, focusSura, focusAya) {
  ...
  // لمسُ الآية يبرزها ويعرض «شرح الآية» — لا يقفز بالقارئ بعيداً عن صفحته
  $('#aya-act').hidden = true;
  $('#m-text').querySelectorAll('.aya').forEach((sp) => {
    sp.onclick = () => {
      $('#m-text').querySelectorAll('.aya.on').forEach((x) => x.classList.remove('on'));
      sp.classList.add('on');
      const s = +sp.dataset.s, a = +sp.dataset.a;
      $('#aya-act-lbl').textContent = suraName(s) + ' — الآية ' + AR_NUM(a);
      $('#aya-act').hidden = false;
      $('#aya-act-tf').onclick = () => setQMode('tafsir', s, a);
    };
  });
  ...
  state.tfPage = 0;                    // الشرحُ يُبنى عند فتح وضعه فقط
  push('page:' + p);
  show('#surah', 'صفحة ' + AR_NUM(p), true);
  await setQMode(qMode(), focusAya ? focusSura : 0, focusAya || 0, true);
  ...
  if (focusAya && qMode() !== 'tafsir') {   // من البحث: تُبرَز الآيةُ المقصودة وتُوسَّط
  ...
  } else if (!focusAya) window.scrollTo(0, 0);   // الصفحةُ الجديدة تبدأ من أعلاها
}

/* ——— أوضاع الصفحة الثلاثة: قراءة · شرح · استماع ———
   الوضعُ يُحفظ ويبقى مع التقليب، فمن اختار الشرح يقلّب صفحاتِ الشرح، ومن اختار
   القراءة لا يرى تحت الصفحة شيئاً. وتلاوةٌ شغّالة لا تنقطع بتبديل الوضع. */
const Q_MODES = ['read', 'tafsir', 'listen'];
const qMode = () => { const m = LS.get('sidrah:qmode', 'read'); return Q_MODES.indexOf(m) >= 0 ? m : 'read'; };

async function setQMode(mode, focusSura, focusAya, quiet) {
  if (Q_MODES.indexOf(mode) < 0) mode = 'read';
  LS.set('sidrah:qmode', mode);
  const v = $('#surah');
  v.dataset.mode = mode;
  v.querySelectorAll('#q-modes .tab').forEach((b) => {
    b.setAttribute('aria-selected', b.dataset.mode === mode ? 'true' : 'false');
    b.onclick = () => { if (qMode() !== b.dataset.mode) setQMode(b.dataset.mode); };
  });
  if (mode === 'tafsir' && state.page && state.tfPage !== state.page.p) {
    await renderTafsir(state.page, (n) => ((state.quran.surahs.find((x) => x.n === n) || {}).name || 'سورة ' + n));
  }
  if (focusAya && mode === 'tafsir') {
    const t = document.getElementById('tf-' + focusSura + '-' + focusAya);
    if (t) { t.classList.add('on'); t.scrollIntoView({ block: 'start', behavior: 'smooth' }); }
  } else if (!quiet) window.scrollTo(0, 0);
}
...
async function renderTafsir(page, suraName) {
  ...
  if (!sel.options.length) {
    ...
    sel.onchange = () => { LS.set('sidrah:tafsir', sel.value); state.tfPage = 0; renderTafsir(state.page, suraName); };
  }
  ...
  if (state.page !== page) return;      // قُلِّبت الصفحةُ أثناء التحميل — لا يُكتب شرحُ صفحةٍ قديمة
  const suras = [...new Set(page.ayat.map((a) => a.s))];
  $('#tf-where').textContent = suras.map(suraName).join(' · ') + ' — صفحة ' + AR_NUM(page.p);
  box.innerHTML = '';
  // الآيةُ بنصّها العثمانيّ ثم معناها تحتها — فلا يُقرأ شرحٌ منفصلٌ عن آيته
  page.ayat.forEach((a) => {
    const t = tafs[a.s] && tafs[a.s][key] ? tafs[a.s][key][String(a.a)] : '';
    if (a.a === 1) box.appendChild(el('div', 'tf-sura', esc(suraName(a.s))));
    const it = el('div', 'tf-item',
      `<div class="tf-aya">${esc(a.t)} <span class="anum">﴿${AR_NUM(a.a)}﴾</span></div>` +
      (t ? `<p>${esc(t)}</p>` : '<p class="empty">لا يتوفّر شرحٌ لهذه الآية.</p>'));
    it.id = 'tf-' + a.s + '-' + a.a;
    box.appendChild(it);
  });
  state.tfPage = page.p;
...
function setupRecite(sura) {
  ...
  const label = (r, extra) => r.name + (extra || r.role ? ' — ' + (extra || r.role) : '');
  if (!sel.options.length) {
    /* مجموعاتٌ في القائمة: مرتّل · مجوّد · المصحف المعلّم. وقيمةُ الخيار رقمُ القارئ في الفهرس
       (لا موضعُه في القائمة) — فاختيارُ من اختار قبل إضافة المجموعات يبقى كما هو. */
    const styles = state.quran.reciter_styles || [{ key: 'murattal', name: 'القرّاء' }];
    sel.innerHTML = styles.map((g) => {
      const opts = state.quran.reciters.map((r, i) => [r, i]).filter(([r]) => (r.style || 'murattal') === g.key)
        .map(([r, i]) => `<option value="${i}">${esc(label(r))}</option>`).join('');
  ...
  state.quran.reciters.forEach((r, i) => {
    ...
    o.textContent = label(r, hasSura(r) ? '' : 'غير مسجَّلة له هذه السورة');
  });
  if (!sel.selectedOptions.length || sel.selectedOptions[0].disabled) sel.value = '0';
  ...
  // تلاوةُ السورة تمتدّ صفحات — فلا يقطعها تقليبُ الصفحة؛ يُزال المشغّلُ المتوقّف فقط
  const au = $('#su-audio audio');
  if (!au || au.paused) $('#su-audio').innerHTML = '';
}
